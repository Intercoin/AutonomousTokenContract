...
Buyers of the token can be assured of the behavior and operation of the smart contracts on the decentralized network, both of the decentralized exchange on which they trade it, and the TradedTokenContract from which they buy it. The source code of the contracts constitutes full disclosure of how their operation. Collateral in the liquidity pool and buyback fund can be employed to secure the value in a completely decentralized, trustless system rather than a common enterprise, so it would be difficult to classify tokens traded in this manner as a security under either the [Howey Test](https://www.sec.gov/news/speech/speech-hinman-061418) or [Risk Capital test](https://www.theselc.org/which_states_apply_the_risk_capital_test_when_deciding_what_is_a_security).
